# Trade API description: https://wex.nz/api/3/docs
...
class TradeAccountInfo(object):
    """
    An instance of this class will be returned by a successful call to TradeAPI.getInfo.
    """
...
CancelOrderResult = namedtuple(
    "CancelOrderResult",
    ["order_id", "funds"],
)

# status: 0 - active, 1 - executed, 2 - canceled, 3 - canceled/partially executed
OrderInfo = namedtuple(
    "OrderInfo",
    ["order_id", "start_amount", "type", "rate", "pair", "timestamp_created", "status", "amount"],
)


def set_history_params(params, from_number, count_number, from_id, end_id, order, since, end):
...
class TradeAPI(object):
    ...
    def _post(self, params, allow_nonce_retry=False):
        ...
        if not success:
            ...
            if "invalid nonce" in err_message:
                ...
                if self.raiseIfInvalidNonce and not allow_nonce_retry:
            ...
            raise Exception("%s call failed with error: %s" % (method, err_message))
    ...
    def trans_history(self, from_number=None, count_number=None,
                      from_id=None, end_id=None, order="DESC",
                      since=None, end=None):

        params = {"method": "TransHistory"}

        set_history_params(params, from_number, count_number, from_id, end_id, order, since, end)
    ...
    def trade_history(self, from_number=None, count_number=None,
    ...
    def order_info(self, order_id):
        ...
        return OrderInfo(order_id, **orders[order_id])
